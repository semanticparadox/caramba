...
pub async fn list_subscriptions(
    ...
    let subs: Vec<AppSubscription> = rows
        ...
        .map(|r| {
            ...
            // used_traffic может быть отрицательным у свежего аккаунта с
            // одноразовым онбординг-headroom (used_traffic засеян в -bonus, см.
            // store_service::grant_onboarding_traffic_tx). Лимит-математика на
            // стороне БД честно использует отрицательное значение как доп. квоту,
            // но в UI "израсходовано" не должно быть отрицательным — клампим к 0.
            let used = used.max(0);
