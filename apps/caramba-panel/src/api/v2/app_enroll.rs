...
//!   * POST /api/v2/app/enroll/redeem   — погашение кода из ссылки
//!     `caramba://` в сессию уже существующего аккаунта;
...
pub(crate) async fn issue_connect_link(state: &AppState, user_id: i64) -> Result<String> {
    ...
    //
    // В этой сборке поля нет никогда, и это не упрощение, а состояние системы:
    // хранилище ключей протокола появляется вместе со своими миграциями, а этот
    // релиз намеренно идёт без единой миграции, чтобы откат оставался заменой
    // бинарника. Пока церемония не проведена, класть сюда нечего, и приложение
    // показывает «не передан» с объяснением — ровно то же, что оно показало бы,
    // прочитав пустое хранилище.
    let root_key_id: Option<[u8; 12]> = None;
