...
async fn main() -> Result<()> {
    println!("Caramba Panel binary started. Version: {}", env!("CARGO_PKG_VERSION"));
    // Load .env
...
async fn run_server(pool: sqlx::PgPool, ssh_public_key: String) -> Result<()> {
    ...
    let api_domain = settings.get_or_default("api_domain", &std::env::var("API_DOMAIN").unwrap_or_else(|_| "your-api-domain.com".to_string())).await;
    ...
    let geo_db_path = std::env::var("GEOIP_DB_PATH").unwrap_or("/usr/share/GeoIP/GeoLite2-Country.mmdb".to_string());
    ...
    let geo_service = Arc::new(services::geo_service::GeoService::new(geo_path_opt.as_deref()));
