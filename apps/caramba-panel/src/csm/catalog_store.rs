...
// `StatusFacts`/`classify` переехали в `caramba-shared`: их зовёт не только
// CSM, но и обычный путь подписки, который обязан собираться без CSM-модуля
// (релиз панели режется от `main`, где этого модуля нет). Реэкспорт оставлен,
// чтобы соседний код в `csm/` не переписывать.
pub use caramba_shared::csm::directive::{StatusFacts, classify};
