...
//!
//!   1 => tstr   connector origin — https-origin, по которому приложение ходит
//!               в API: зеркало (`subscription_domain`), если оно настроено,
//!               иначе сама панель. Пример: "https://app.exarobot.top"
...
//! отправителя, которое приложение рисует строкой на экране подтверждения.
//! Перевод строки внутри имени даёт вторую строку, визуально неотличимую от
//! настоящей строки экрана, а bidi-override переставляет
...
pub struct ConnectProfile {
    /// https-origin, к которому приложение пойдёт за всем остальным: зеркало,
    /// если оно настроено, иначе сама панель (см. `app_enroll::connector_origin`).
...
/// поэтому погашение не нуждается в rate limit).
...
mod tests {
    ...
    fn sample() -> ConnectProfile {
        ConnectProfile {
            // Домен зеркала, а не панели: ссылка несёт именно его (см.
            // `app_enroll::connector_origin`), и пример обязан это показывать.
...
1050R3GG28A1C60T3GF0DQM6RBJC5PP4R908DQPWVK5CDT0A6KA32JG0063SHSG"
    ...
    fn over_long_field_is_refused_at_issuance() {
        ...
        // Origin коннектора проверяется тем же гейтом: он приходит из тех же
    ...
    ///
    /// Ровно эта строка на экране подтверждения дорисовывает вторую строку,
    /// визуально неотличимую от настоящей строки экрана.
    #[test]
