...
pub async fn get_updates_page(
    ...
    let panel_latest = state.settings.get_or_default("panel_latest_version_cache", &panel_current).await;
    ...
    let auto_update_agents = state.settings.get_bool_or_default("auto_update_agents", true).await;

    let active_nodes = state.infrastructure_service.get_active_nodes().await.unwrap_or_default();
...
pub async fn trigger_update(
    ...
    let target_version = state.settings.get_or_default("agent_latest_version", "").await;
    if target_version.is_empty() {
        return (axum::http::StatusCode::BAD_REQUEST, "No target agent version configured").into_response();
    }
    ...
    if let Some(node_id) = form.target_id {
        ...
        for node in active_nodes {
            let _ = sqlx::query("UPDATE nodes SET target_version = $1 WHERE id = $2")
                ...
                .execute(&state.pool)
                .await;
            if state.orchestration_service.notify_node_update(node.id).await.is_ok() {
                count += 1;
            }
        }
        return (axum::http::StatusCode::OK, format!("Rollout signal sent to {} nodes", count)).into_response();
    }
}
...
async fn fetch_worker_inventory_shared(pool: &sqlx::PgPool) -> Vec<crate::handlers::admin::settings::WorkerInventoryView> {
    ...
    rows.into_iter().map(|row| {
        let is_online = chrono::Utc::now().signed_duration_since(row.last_seen) <= chrono::Duration::minutes(3);
        WorkerInventoryView {
            ...
            is_online,
            online_label: if is_online { "online".into() } else { "offline".into() },
