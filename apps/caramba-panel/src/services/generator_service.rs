...
impl GeneratorService {
    ...
    async fn ensure_inbound_exists(&self, node: &Node, template: &InboundTemplate) -> Result<()> {
        ...
        let existing_inbound: Option<(i64, i64)> = sqlx::query_as(
            "SELECT id, listen_port FROM inbounds WHERE node_id = $1 AND tag = $2"
        )
        .bind(node.id)
        .bind(&tag)
    ...
    pub async fn rotate_inbound(&self, inbound_id: i64) -> Result<()> {
        let inbound = sqlx::query_as::<_, caramba_db::models::network::Inbound>("SELECT * FROM inbounds WHERE id = $1")
            .bind(inbound_id)
            .fetch_one(&self.pool)
            .await
        ...
        let new_port = self.orchestration_service.allocate_port(inbound.node_id, template.port_range_start, template.port_range_end).await?;
