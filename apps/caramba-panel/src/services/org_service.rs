...
use caramba_db::models::orgs::{Organization, OrgRole};
...
impl OrganizationService {
    ...
    pub async fn create_organization(&self, owner_id: i64, name: &str, slug: Option<&str>) -> Result<Organization> {
        let org_id = self.org_repo.create(name, slug).await?;
        self.org_repo.add_member(org_id, owner_id, &OrgRole::Owner.to_string()).await?;
        
        self.org_repo.get_by_id(org_id).await?
    ...
    pub async fn add_admin(&self, org_id: i64, user_id: i64) -> Result<()> {
        self.org_repo.add_member(org_id, user_id, &OrgRole::Admin.to_string()).await
    }

    pub async fn add_member(&self, org_id: i64, user_id: i64) -> Result<()> {
        self.org_repo.add_member(org_id, user_id, &OrgRole::Member.to_string()).await
    }
}
