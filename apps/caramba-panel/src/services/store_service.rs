...
impl StoreService {
    ...
    pub async fn get_products_by_category(&self, category_id: i64) -> Result<Vec<caramba_db::models::store::Product>> {
    ...
    pub async fn create_api_key(&self, name: &str, key: &str, max_uses: Option<i64>) -> Result<caramba_db::models::api_key::ApiKey> {
    ...
    pub async fn get_user_nodes(&self, user_id: i64) -> Result<Vec<caramba_db::models::node::Node>> {
    ...
    pub async fn create_family_invite(&self, parent_id: i64, max_uses: i32, duration_days: i32) -> Result<caramba_db::models::store::FamilyInvite> {
    ...
    pub async fn get_valid_invite(&self, code: &str) -> Result<Option<caramba_db::models::store::FamilyInvite>> {
        let invite = sqlx::query_as::<_, caramba_db::models::store::FamilyInvite>(
    ...
    pub async fn add_bot_message_to_history(&self, user_id: i64, chat_id: i64, message_id: i32) -> Result<()> {
        sqlx::query("INSERT INTO bot_chat_history (user_id, chat_id, message_id) VALUES ($1, $2, $3)")
            .bind(user_id)
            .bind(chat_id)
            .bind(message_id)
            .execute(&self.pool)
    ...
    pub async fn purchase_plan(&self, user_id: i64, duration_id: i64) -> Result<Subscription> {
        ...
        let duration = sqlx::query_as::<_, caramba_db::models::store::PlanDuration>("SELECT * FROM plan_durations WHERE id = $1")
            .bind(duration_id)
            .fetch_one(&mut *tx)
            .await?;
    ...
    pub async fn transfer_subscription(&self, sub_id: i64, current_user_id: i64, target_username: &str) -> Result<Subscription> {
        ...
        let sub = sqlx::query_as::<_, Subscription>("SELECT * FROM subscriptions WHERE id = $1 AND user_id = $2 FOR UPDATE")
            .bind(sub_id)
        ...
        let target_user = sqlx::query_as::<_, caramba_db::models::store::User>("SELECT * FROM users WHERE username = $1")
    ...
    pub async fn admin_refund_subscription(&self, sub_id: i64, amount: i64) -> Result<()> {
        let mut tx = self.pool.begin().await?;
        let sub = self.sub_repo.get_by_id(sub_id).await?.context("Subscription not found")?;
        self.sub_repo.delete(sub_id).await?;
        sqlx::query("UPDATE users SET balance = balance + $1 WHERE id = $2")
            .bind(amount)
            .bind(sub.user_id)
    ...
    pub async fn admin_gift_subscription(&self, user_id: i64, plan_id: i64, duration_days: i32) -> Result<Subscription> {
        ...
        let node_id = active_nodes.first().cloned().ok_or_else(|| anyhow::anyhow!("No active nodes available"))?;
    ...
    pub async fn extend_subscription(&self, user_id: i64, duration_id: i64) -> Result<Subscription> {
        ...
        let duration = sqlx::query_as::<_, PlanDuration>("SELECT * FROM plan_durations WHERE id = $1")
            .bind(duration_id).fetch_one(&mut *tx).await?;
        ...
        sqlx::query("UPDATE users SET balance = balance - $1 WHERE id = $2")
            .bind(duration.price).bind(user_id).execute(&mut *tx).await?;
    ...
    async fn extend_subscription_with_duration_internal(&self, user_id: i64, duration: &caramba_db::models::store::PlanDuration, _tx: &mut sqlx::Transaction<'_, sqlx::Postgres>) -> Result<Subscription> {
        ...
        let sub = if let Some(active_sub) = existing_sub {
            if active_sub.plan_id != duration.plan_id {
                 ...
                 let vless_uuid = Uuid::new_v4().to_string();
                 let sub_uuid = Uuid::new_v4().to_string();
                 let id = self.sub_repo.create(user_id, duration.plan_id, &vless_uuid, &sub_uuid, expires_at, "active", None, false).await?;
    ...
    pub async fn update_subscription_note(&self, sub_id: i64, note: String) -> Result<()> {
        sqlx::query("UPDATE subscriptions SET note = $1 WHERE id = $2").bind(note).bind(sub_id).execute(&self.pool).await.context("Failed to update subscription note")?;
        Ok(())
    }
    ...
    pub async fn apply_referral_bonus(&self, pool: &mut sqlx::Transaction<'_, sqlx::Postgres>, user_id: i64, amount_cents: i64, payment_id: Option<i64>) -> Result<Option<(i64, i64)>> {
    ...
    pub async fn create_category(&self, name: &str, description: Option<&str>, sort_order: Option<i32>) -> Result<()> {
        sqlx::query("INSERT INTO categories (name, description, sort_order) VALUES ($1, $2, $3)").bind(name).bind(description).bind(sort_order).execute(&self.pool).await?;
        Ok(())
    }

    pub async fn delete_category(&self, id: i64) -> Result<()> {
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM products WHERE category_id = $1").bind(id).fetch_one(&self.pool).await.unwrap_or(0);
        if count > 0 { return Err(anyhow::anyhow!("Cannot delete category with existing products")); }
    ...
    pub async fn get_all_products(&self) -> Result<Vec<caramba_db::models::store::Product>> {
        sqlx::query_as::<_, caramba_db::models::store::Product>("SELECT * FROM products ORDER BY created_at DESC").fetch_all(&self.pool).await.context("Failed to fetch all products")
    }
    ...
    pub async fn validate_promo(&self, code: &str) -> Result<Option<caramba_db::models::promo::PromoCode>> {
    ...
    pub async fn checkout_cart(&self, user_id: i64) -> Result<()> {
        ...
        if cart.is_empty() { return Err(anyhow::anyhow!("Cart is empty")); }
        let total_price: i64 = cart.iter().map(|item| item.price * item.quantity).sum();
        let mut tx = self.pool.begin().await?;
        let user = sqlx::query_as::<_, User>("SELECT * FROM users WHERE id = $1 FOR UPDATE").bind(user_id).fetch_one(&mut *tx).await?;
        if user.balance < total_price { return Err(anyhow::anyhow!("Insufficient balance")); }
        sqlx::query("UPDATE users SET balance = balance - $1 WHERE id = $2").bind(total_price).bind(user_id).execute(&mut *tx).await?;
    ...
    pub async fn clear_cart(&self, user_id: i64) -> Result<()> {
        sqlx::query("DELETE FROM cart_items WHERE user_id = $1").bind(user_id).execute(&self.pool).await?;
        Ok(())
    }
}
