...
impl PayService {
    ...
    pub async fn get_bot_username(&self) -> String {
        self.bot_manager.get_username().await.unwrap_or_else(|| "YOUR_BOT_USERNAME".to_string())
    }
    ...
    pub async fn create_cryptomus_invoice(&self, user_id: i64, amount_usd: f64, payment_type: PaymentType) -> Result<String> {
        if let Some(adapter) = self.adapters.get("cryptomus") {
            ...
            adapter.create_invoice(user_id, amount_usd, payment_type, &bot_user, &self.api_domain).await
        } else {
