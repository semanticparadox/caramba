...
impl OrchestrationService {
    ...
    pub async fn init_default_inbounds(&self, node_id: i64) -> anyhow::Result<()> {
        ...
        // 1. Get Node's Groups
        let group_ids: Vec<i64> = match sqlx::query_scalar(
        ...
        if templates.is_empty() {
            info!(
                "No templates found for node groups. Bootstrapping default templates for 'Default' group..."
            );
            ...
            if let Some(group) = default_group {
                ...
                templates = self
                    .node_repo
                    .get_templates_for_group(group.id)
                    .await
                    .unwrap_or_default();
            }
        }
        ...
        if let Err(e) = self.node_repo.link_node_inbounds_to_plan(1, node_id).await {
            warn!(
                "Failed to link default plan to node {} inbounds: {}",
                node_id, e
            );
        }
    ...
    async fn ensure_minimal_default_inbound(&self, node: &Node) -> anyhow::Result<()> {
        ...
        let listen_port = if effective_node.vpn_port > 0 {
            effective_node.vpn_port
        } else {
            443
        };
        ...
        if let Err(e) = insert_result {
            warn!(
                "Fallback inbound upsert failed for node {}: {}. Trying minimal compatibility insert.",
                effective_node.id, e
            );

            // Compatibility path for legacy schemas lacking new columns/unique constraints.
            let _ = sqlx::query(
                "UPDATE inbounds SET protocol = $1, settings = $2, stream_settings = $3 WHERE node_id = $4 AND tag = $5",
            )
            .bind("vless")
            .bind(&settings)
            .bind(&stream_settings)
            ...
            {
                warn!(
                    "Minimal fallback inbound insert failed for node {}: {}",
                    effective_node.id, e2
                );
            }
        }
        ...
        let _ = self
            .node_repo
            .link_node_inbounds_to_plan(1, effective_node.id)
            .await;
        Ok(())
    }
    ...
    async fn instantiate_inbound_from_template(
        &self,
        node: &Node,
        template: &caramba_db::models::groups::InboundTemplate,
    ) -> anyhow::Result<()> {
        info!(
            "Instantiating template '{}' for node {}",
            template.name, node.id
        );
    ...
    fn generate_reality_keys(&self) -> anyhow::Result<(String, String, String)> {
        ...
        let priv_key = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .encode(secret.to_bytes())
            .trim()
            .to_string();
        let pub_key = base64::engine::general_purpose::URL_SAFE_NO_PAD
    ...
    pub async fn allocate_port(&self, node_id: i64, start: i64, end: i64) -> anyhow::Result<i64> {
        ...
        Err(anyhow::anyhow!(
            "Failed to allocate port for node {} in range {}-{}",
            node_id,
            start,
            end
        ))
    ...
    pub async fn generate_node_config_json(
        &self,
        node_id: i64,
    ) -> anyhow::Result<(caramba_db::models::node::Node, serde_json::Value)> {
        ...
        if is_invalid_key {
            ...
            let has_reality = inbounds
                .iter()
                .any(|i| i.stream_settings.contains("reality"));
            if has_reality {
                info!(
                    "✨ Lazily generating or HARD-FIXING Reality keys for node {}",
                    node.id
                );
        ...
        for inbound in &mut inbounds {
            ...
            if inbound.stream_settings.contains("{{") {
                inbound.stream_settings = inbound
                    .stream_settings
                ...
                if let Some(sni) = &node.reality_sni {
                    ...
                    inbound.stream_settings =
                        inbound.stream_settings.replace("{{sni}}", "www.google.com");
                }
            }
            ...
            if let Ok(mut stream) = serde_json::from_str::<
                caramba_db::models::network::StreamSettings,
            >(&inbound.stream_settings)
            {
        ...
        for inbound in &mut inbounds {
            if !inbound.enable {
                warn!(
                    "⚠️ Inbound {} is DISABLED, skipping user injection",
                    inbound.tag
                );
                continue;
            }
            ...
            if linked_plans.is_empty() {
                warn!(
                    "⚠️ Inbound {} has NO linked plans for node {}, users will be empty",
                    inbound.tag, node_id
                );
            }
            ...
            info!(
                "Found {} active subscriptions for inbound {}",
                active_subs.len(),
                inbound.tag
            );
            ...
            match serde_json::from_value::<InboundType>(settings_value) {
                ...
                Err(e) => {
                    error!(
                        "Skipping user injection for inbound {} due to parse error: {}",
                        inbound.tag, e
                    );
                }
            }
        ...
        if let Err(e) = ConfigGenerator::validate_config(&config) {
            error!(
                "❌ Generated config for node {} FAILED VALIDATION: {}",
                node_id, e
            );
            return Err(e);
        }
