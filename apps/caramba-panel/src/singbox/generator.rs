        ...
        SingBoxConfig {
            ...
            experimental: Some(ExperimentalConfig {
                ...
                v2ray_api: {
                    ...
                    // Два условия, и второе — предохранитель.
                    //
                    // Секцию понимает только sing-box, собранный с
                    // `with_v2ray_api`. Официальная сборка SagerNet идёт без
                    // него и ОТКАЗЫВАЕТСЯ СТАРТОВАТЬ, встретив её. Поэтому
                    // пишем её лишь тем узлам, которые сами сообщили о своей
                    // способности в heartbeat: иначе выкат панели раньше узлов
                    // положил бы VPN сразу всем.
                    //
                    // Побочный эффект приятный: раскатка идёт пошагово сама
                    // собой — узел получает секцию ровно тогда, когда научился
                    // её понимать, и порядок обновления перестаёт иметь значение.
                    if users.is_empty() || !node.supports_v2ray_api {
                        None
                    } else {
                        Some(V2RayApiConfig {
