...
///
/// Флаг ставит диплинк импорта (`carambaconnect://import`) — и только он:
/// руками «выбрать generic-режим» больше негде, гейт в шелл пускает всех.
/// Пережить перезапуск флаг обязан потому, что он же говорит роутеру не
/// ждать auth-пробу, пока своих профилей ещё не видно. Вход в панель флаг
/// не снимает: аккаунт и своя подписка сосуществуют как разные профили
/// подключения.
...
///
/// Достаточно ЛЮБОГО из двух: поднятый диплинком импорта generic-режим или
/// хотя бы один заведённый профиль подключения (импортированная подписка).
/// Второе условие важно для установок, где профиль появился раньше самого
/// флага, и для импорта по deeplink.
...
/// его, иначе холодный старт с профилем успевал бы застрять на сплеше.
