...
///
/// ПРАВИЛО ОДНОЙ ФРАЗОЙ. Применяется САМО всё, что отвечает на вопрос «куда и
/// как идёт трафик» — страна/узел выхода, relay, тип подключения и РЕЖИМ
/// (пресет), — независимо от экрана, на котором стоит контрол: режим владелец
/// перенёс в Настройки (раздел «Правила трафика»), но ключ политики у него
/// прежний, путевой (`preset`), и применяется он по-прежнему сам. Остальное в
/// Настройках (реклама, правила по сайтам, DNS, стек, MTU, IPv6, FakeIP,
/// режим захвата, kill switch) ждёт кнопку «Переподключить».
///
/// ПОЧЕМУ ИМЕННО ЭТИ ЧЕТЫРЕ. Страну и тип назвал владелец. Relay и режим
/// оказались с ними даже не на одном экране, а в одной группе СМЫСЛА: они
/// отвечают на тот же вопрос «куда и как идёт трафик», и один контрол из
/// четырёх, ведущий себя иначе, — сюрприз. Режим особенно: человек переключает
/// «Полный обход» и смотрит в браузер, а не в баннер под дайлом.
///
