...
class InboundLatency {
  ...
  String? get verdictNote {
    ...
    return switch (verdict) {
      ...
      ProbeVerdict.unknown => answered > 0
  ...
  /// Строку предлагает больше одной машины, и число — лучшая из ответивших.
  /// Об этом надо сказать словами: иначе число читается как замер входа, к
  /// которому подключишься, а подключишься не обязательно к нему.
  String? get spreadNote {
    if (named < 2) return null;
    if (answered == 0) {
      return measured == 0
          ? null
          : 'Ни один из $named узлов с этим входом не пропустил запрос.';
    }
    return 'Это лучший из $answered ответивших узлов с таким входом '
        '(всего их $named).';
  }
}
