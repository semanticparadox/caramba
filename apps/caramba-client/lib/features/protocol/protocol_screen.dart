...
class _ProtocolScreenState extends ConsumerState<ProtocolScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Тот же провайдер, что и у строки на «Подключении»: расхождение типов
    // обязано называться на обоих экранах одними словами.
      ...
      body: SafeArea(
        ...
        child: ListView(
          ...
          children: [
            ...
            // Расхождение стоит ПЕРВЫМ и тем же текстом, что на
            // «Подключении». Человек приходит сюда по строке, которая
            // только что сказала «VLESS вместо TUIC», — и видит выделенным
            // TUIC. Без этой фразы выделение читалось бы как опровержение
            // «Подключения», а два экрана, спорящих о туннеле, хуже одного
            // молчащего.
            if (truth.diverged) ...[
            ...
            if (slate.scope == ProtocolScope.wholeFleet) ...[
              ...
              const InlineBanner(
                ...
                text:
                    ...
                    'доступных узлов. Выберите сервер по строке «Сервер» на '
                    '«Подключении» — и список станет инбаундами именно его.',
              ),
            ],
            if (entry != null) ...[
              const SizedBox(height: AppSpace.s3),
...
///
/// Тем же приёмом собрана кнопка автоподбора на «Подключении»
...
/// все случаи сразу расходилось с «Подключением» и «Серверами», которые в
/// том же состоянии говорят «не в силе»: расхождение с держателем свежий
/// замер не лечит, и слово «устарело» посылало перезамерять там, где надо
/// снять пин. Два экрана, называющие одно состояние разными словами, — это
/// два разных состояния для того, кто на них смотрит.
