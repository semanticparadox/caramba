...
///
/// АДРЕСА ПАНЕЛИ В ЭТОМ ПОТОКЕ НЕТ НИГДЕ — РЕШЕНО ЗДЕСЬ, ОДИН РАЗ.
/// Решение владельца (раунд 5): приложение не показывает адрес панели ни на
/// одном экране и ни под каким действием. Причина не косметическая. Ссылка
/// теперь несёт origin ЗЕРКАЛА, а не самой панели, и весь смысл этой замены в
/// том, чтобы адрес панели не всплывал ни в переписке, ни на экране, ни на
/// скриншоте в поддержку. Кнопка «Показать адрес панели» возвращала бы туда
/// ровно то, что убрали на стороне панели, поэтому её здесь больше нет — как
/// нет и строки «Корневой ключ», которая раскрывалась вместе с ней.
///
///   • [_confirm] — имя из ссылки и остаток срока приглашения. Один баннер: он
///     говорит про саму ссылку (не зашифрована, кто получил, тот и подключится)
///     и касается каждого, кто её открыл.
///
///   • [_done] — имя оператора и статус подписки. Кадр висит дольше всех (при
///     проблеме с подпиской — до нажатия кнопки), и именно его отправляют в
///     поддержку скриншотом; адреса в нём нет и не было.
///
/// Тому, кто придёт следующим: «вернуть для сверки» строку с адресом нельзя ни
/// на дороге, ни под кнопкой. Сверять адрес по-прежнему есть чем, но не текстом
/// в приложении: на этом экране проверяется целостность байт ссылки и то, что
/// адрес по https, а отпечаток корневого ключа оператора живёт на отдельном
/// экране личности оператора (`/csm/operator`), где адреса нет. Сторожит
/// test/panel_address_exposure_test.dart: origin не появляется ни на одном
/// кадре потока.
library;
...
class _ConnectScreenState extends ConsumerState<ConnectScreen> {
  ...
  List<Widget> _confirm(BuildContext context, ConnectState s) {
    ...
    return [
      ...
      // Единственный баннер экрана. Он про саму ссылку, а не про сверку адреса:
      // сверять текстом в приложении больше нечего (см. шапку файла), а знать,
      // что ссылка одноразовая и непереносимая, обязан каждый, кто её получил.
      const InlineBanner(
        glyph: Lucide.key,
        text:
            'Ссылка не зашифрована и не подписана: кто её получил, тот и '
            'подключится, поэтому не пересылайте её. Подключайтесь только по '
            'ссылке, которую прислал ваш оператор.',
      ),
      const SizedBox(height: AppSpace.s5),
      FilledButton(
