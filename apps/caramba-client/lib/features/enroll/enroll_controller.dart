...
/// потока. Регистрация идёт через ОТДЕЛЬНЫЙ [ApiClient], нацеленный на URL
/// панели из ссылки (а не на дефолтный тенант-1 [kApiBaseUrl]) — иначе токены
/// выпустит не тот инстанс (риск из recon).
...
enum EnrollStage {
  /// Ссылки нет или она не разобралась: экран объясняет, где её берут.
  /// Ручного ввода кода и адреса панели больше не существует (раунд 5).
  needInput,

  /// Идёт публичная валидация кода на панели.
  validating,

  /// Код валиден: показываем имя панели, онбординг-трафик и регистрацию.
  valid,
...
class EnrollNotifier extends StateNotifier<EnrollState> {
  ...
  Future<void> openDeeplink(String raw) async {
    ...
    if (link == null) {
      state = state.copyWith(
        ...
        error: 'Ссылка энроллмента не распознана. Попросите у оператора новую.',
      );
      return;
    }
  ...
  /// Принимает код + адрес панели, разобранные из ссылки энроллмента.
  ///
  /// Имя историческое: ручного ввода, ради которого метод и появился, больше
  /// нет, а единственный вызывающий это экран, получивший обе части диплинком.
  Future<void> submitManual({
    ...
    if (link == null) {
      state = state.copyWith(
        stage: EnrollStage.needInput,
        error: 'Ссылка энроллмента неполная или не по https.',
      );
      return;
    }
  ...
  /// Общий путь создания аккаунта на панели из ссылки. Токены кладём в общий
  /// [TokenStore] и перенимаем сессию инвалидацией [authProvider].
  ///
  /// Обобщение пережило второй способ входа (6-значный код из бота), который
  /// удалён вместе с режимом кода: форма осталась, потому что разбор ответа,
  /// запись кредов и регистрация в CSM одинаковы для любого исхода авторизации,
  /// и следующий способ ляжет сюда же.
