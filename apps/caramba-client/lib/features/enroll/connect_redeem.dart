...
Future<ConnectRedeemResult> redeemConnectCode({
  ...
  try {
    ...
    res = e.response;
    if (res == null) {
      // Хоста в тексте нет намеренно: этот текст видит пользователь, а адрес
      // панели приложение не показывает нигде (см. шапку connect_screen.dart).
      // Для поддержки остаётся тип сетевого отказа — по нему и различают
      // «нет сети» от «панель не отвечает».
