...
/// «Подписка» на «Подключении» и в списке подключений. Ни одно из этих мест
/// ничего им не подтверждает — там он просто написан.
///
...
/// Экран «Добавить подключение»: одно поле, QR и кнопка «Продолжить».
///
/// Тонкая обёртка вокруг [ConnectionEntryForm] — вся работа в форме. Форма
/// живёт только здесь: единственный разбор вставленной строки означает, что
/// `caramba://`, конфиг и ссылка на подписку ведут себя одинаково, откуда бы
/// человек ни пришёл.
...
class ConnectionEntryForm extends ConsumerStatefulWidget {
  ...
  /// Шапка экрана. Приходит снаружи: форма не знает, кто её показывает.
  final Widget head;

  /// Дополнительные пункты под «Ещё». Единственный хозяин формы — экран
  /// «Добавить подключение», и ему они не нужны; параметр оставлен для
  /// брендированных сборок, которым понадобится свой вход.
  final List<Widget> extras;

  /// Вызывается ПЕРЕД сохранением профиля. Единственному хозяину формы —
  /// экрану «Добавить подключение» — здесь делать нечего; параметр оставлен
  /// для брендированных сборок.
