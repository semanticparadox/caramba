...
/// оператор не предлагает. Теперь единственная точка входа — строка «Режим» в
/// Настройках (раздел «Правила трафика»); с «Подключения» строка убрана
/// владельцем.
///
...
/// Подпись строки «Режим» в Настройках: флаг страны списка блокировок (если
/// у пресета он есть) и имя режима. Флаг тот же, что в листе, — из зеркала
/// реестра ядра, а не набранный заново.
String routeModeLabel(RoutingMode mode) {
  final p = routePresetForMode(mode.id);
  if (p != null && p.emoji.isNotEmpty && p.countryCode.isNotEmpty) {
    return '${p.emoji} ${mode.name}';
  }
  return mode.name;
}
