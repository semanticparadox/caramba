...
class Relay {
  ...
  /// Собирает список для пикера: Выкл / Авто + relay-страны с панели.
  ...
  static List<Relay> fromCountries(List<Relay> countries) {
    ...
    return <Relay>[...defaults, ...countries];
  }
  ...
  ///
  /// Настоящие входы теперь приходят от источника через слой предложения
  /// (`domain/offering`, `RelayOffer`) — узлами, а не выдуманными странами.
  /// Здесь остались только две строки, истинные при любом флоте и не
  /// описывающие ни одной страны: «Выкл» — прямое подключение к выбранному
  /// выходу (ядру уходит явное «без релея»), «Авто» — решение остаётся за
  /// оператором (ядру уходит пустая строка). Это разные намерения, поэтому
  /// строки две, а не одна.
  static const defaults = <Relay>[
    Relay(name: 'Выкл', desc: 'Прямое подключение к выбранному серверу.'),
    Relay(id: 'auto', name: 'Авто', desc: 'Вход выбирает оператор.'),
  ];
}
