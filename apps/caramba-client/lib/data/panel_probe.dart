...
class PanelProbeResult {
  ...
  /// Имя оператора, как его опубликовала САМА панель. Пусто, когда оператор
  /// брендинг не заполнил.
  ///
  /// ХОСТА ЗДЕСЬ БОЛЬШЕ НЕТ, И ЭТО НЕ УПРОЩЕНИЕ. Геттер назывался `displayName`
  /// и при пустом бренде подставлял хост панели. Подстановка выглядела
  /// безобидной, но она принимала за вызывающего решение, которое вызывающий
  /// обязан принимать сам: адрес панели это материал экрана подтверждения, где
  /// он ПРОВЕРЯЕМ и нужен для решения, а не запасной ярлык везде, где не нашлось
  /// имени. На практике она означала, что у любого оператора без брендинга
  /// адрес его панели печатался в заголовке листа импорта.
  ///
  /// Пустая строка заставляет каждое место решать заново — и объяснять решение.
  /// [origin] никуда не делся: кому адрес нужен по делу, тот берёт его явно.
  String get brandName => branding.brandName.trim();
}
