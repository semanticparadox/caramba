...
abstract final class AppRoute {
  ...
  /// Автоподбор, запущенный ПОВТОРНО из приложения (Подключение, Настройки,
  /// экран серверов).
  ///
  ...
  /// ветку настроек он именно это и делал: человек уходил с «Подключения», а
  /// возвращался в Настройки — и следующее «Назад» закрывало приложение.
  static const settingsAutotune = '/settings/autotune';

  // Нижняя навигация: Подключение / Профиль / Настройки. /servers — накладной
  // экран со строки «Сервер».
  static const home = '/home';

  /// Список машин оператора. Накладной экран (открывается со строки «Сервер»
  /// на «Подключении»); вкладкой быть перестал по решению владельца.
  ...
  /// вещи разом — блок рекламы, списки сайтов и режим страны; режим уехал в
  /// Настройки строкой «Режим» того же раздела, реклама уехала туда же, и от
  ...
  /// в Настройки, списки сайтов стали [siteRules], режим — в Настройки.)
  ///
  ...
  ///
  /// Сплеш и первый `/autotune` сюда НЕ входят намеренно: они не лежат поверх
  /// приложения, они его заменяют, и стека под ними нет по смыслу. А вот
  /// `/enroll` и `/connect` ВХОДЯТ: их открывают из шелла («Добавить
  /// подключение», «Аккаунт панели»), и «Назад» обязано возвращать туда,
  /// откуда пришли. На холодном старте по диплинку шелла под ними нет —
  /// `CarambaRouter.opensOverStack` сам заменяет экран, а не кладёт поверх
  /// (первый матч не `ShellRouteMatch`). `/login` входит по той же причине: из
  /// шелла на него уходят по своей воле («Войти или подключить панель»).
