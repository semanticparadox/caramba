...
class FleetFact {
  ...
  /// Прокси — ВХОД цепочки, а не выход.
  ///
  /// Приезжает из предложения ([InboundOffer.role]), где ставится только по
  /// прямому свидетельству источника: `role: relay` у ядра (оно выводит роль из
  /// `dialer-proxy`/`detour`) или встречная ссылка `via_relay` у панели.
  /// Молчание источника оставляет `false` — иначе автоподбор молча выбрасывал
  /// бы рабочие узлы, про которые просто ничего не сказано.
  ///
  /// Отдельно от [wireChained]: то поле про УЗЕЛ, к которому ведёт цепочка,
  /// это — про сам промежуточный узел. Их легко перепутать, и цена ошибки
  /// разная: там подпись, здесь страна, из которой человек выйдет в интернет.
  final bool isRelay;
...
class AutoCandidate {
  ...
  /// Узел годится в ВЫХОД. Единственное, что здесь запрещает, — заведомая роль
  /// входа: через такой узел набирают другие, и выйти в интернет через него
  /// значит выйти из его страны.
  bool get routable => !fact.isRelay;
...
enum AutoFailureKind {
  ...
  /// Проверку прошли только узлы с ролью ВХОДА. Выхода среди них нет ни одного,
  /// и подставить вход вместо выхода нельзя даже как «лучше, чем ничего»:
  /// человек оказался бы в интернете из страны входа.
  onlyRelays,
}
  ...
  String get message => switch (kind) {
    ...
    AutoFailureKind.mixed =>
      'Ни один узел не пропустил проверочный запрос. Причины у разных узлов '
          'разные.',
    AutoFailureKind.onlyRelays =>
      'Проверку прошли только промежуточные узлы (входы цепочки). Выходом '
          'ни один из них не является: подключение через такой узел вывело бы '
          'вас в интернет из его страны.',
  };
...
AutoPickOutcome autoPick({
  ...
  // Узел с ролью ВХОДА исключается из победы первым и без всяких «если».
  //
  // Это единственное исключение в файле, у которого нет запасного выхода: у
  // остальных правил есть откат к полному списку, потому что цена ошибки —
  // подпись или лишние миллисекунды. Здесь цена другая: релэй, выбранный как
  // выход, отправляет человека в интернет из страны входа — ровно из той, от
  // которой он и уходил, ставя VPN. Поэтому «самый быстрый» тут не аргумент, и
  // пустой список честнее подмены.
  final routable = ranked.where((c) => c.routable).toList(growable: false);
  if (routable.isEmpty) {
    return AutoPickOutcome(
      failure: AutoFailure(
        kind: AutoFailureKind.onlyRelays,
        tally: tallyVerdicts(considered),
      ),
      ranked: ranked,
      rejected: rejected,
  ...
  // худший из возможных исходов. Откат идёт к [routable], а НЕ к полному
  // списку: страховка от собственной ошибки не имеет права вернуть в игру
  // релэй.
  final eligible = routable
      .where((c) => !c.chainDisprovedByTwin)
      .toList(growable: false);
  final pool = eligible.isEmpty ? routable : eligible;
  final best = _preferHonestName(pool);
  final String bestReason;
  if (identical(best, ranked.first)) {
    bestReason = 'best_score';
  } else if (!ranked.first.routable) {
    // Лидер прохода оказался входом цепочки. Победил не «второй по счёту», а
    // первый, кем вообще можно выйти, и называться это обязано отдельно:
    // иначе разбор итога приписал бы смену лидера близнецу, которого тут нет.
    bestReason = 'relay_skipped';
...
String _plainTwinNameOf(AutoCandidate c, List<AutoCandidate> all) {
  ...
  for (final o in all) {
    ...
    // Вход цепочки близнецом не бывает: он другой провод и другая страна.
    // Назвать его «тем же самым» значило бы предложить человеку выйти через
    // него — тем же способом, каким предлагают честную замену.
    if (!o.routable) continue;
