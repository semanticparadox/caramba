...
class AutoLabel {
  ...
  ///
  /// Словарь общий с «Подключением» (home_screen.dart, `_autopilotRow`): два
...
/// Узел, который В СИЛЕ, — вторая половина пары фактов на «Подключении».
///
...
/// Текст баннера автоподбора на «Подключении».
///
...
/// «Авто · DE · Германия — Сейчас в туннеле», а «Подключение» в ту же
/// секунду — «Автоподбор выбрал Канада… не в силе». Два экрана называли
/// один выбор по-разному, потому что смотрели в разные источники.
///
  ...
  Future<bool> run() async {
    ...
    if (profile == null) {
      state = const AutopilotRun(
        error:
            'Нет активного профиля. Добавьте подключение или подключите панель.',
      );
      return false;
    }
