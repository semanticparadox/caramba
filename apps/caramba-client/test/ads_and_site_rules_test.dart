// Правила трафика: где включаются реклама и блокировки, и что при этом
// обещано человеку.
//
...
// экране «Правила по сайтам», режим — строкой выше в том же разделе.
//
...
void main() {
  ...
  group('siteRulesSummary', () {
    ...
    // Режим живёт в Настройках отдельной строкой и подписан там своим
    // именем. Повторить его здесь значило бы снова завести два места для
    // одной величины — ровно то, что владелец и просил убрать.
  ...
  group('реклама включается ровно в одном месте', () {
    ...
    testWidgets('в Настройках — есть', (tester) async {
      ...
      expect(
        find.text('Режим'),
        findsOneWidget,
        reason: 'режим переехал в этот раздел',
      );
  ...
  group('экран «Правила по сайтам»', () {
    testWidgets('называется по назначению и несёт только списки', (
      ...
      // Режим и рекламу забрали Настройки: ни того, ни другого раздела
  ...
  group('раздел «Правила трафика» не обрезает подписи', () {
