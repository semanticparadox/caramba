...
//
// И отдельно: экран обязан говорить про саму ссылку. Адреса панели на нём нет
// нигде (раунд 5: приложение не показывает его ни под каким действием), а имя
// не проверяет никто, и подпись строки называет его происхождение.
...
void main() {
  ...
  testWidgets('экран разделяет проверяемый адрес и заявленное имя', (
    ...
    // Адреса нет, и раскрыть его нечем: кнопка «Показать адрес панели» убрана
    // вместе со строкой, а совет «сверяйте адрес» над экраном, где адреса нет,
    // был бы советом ни о чём.
    ...
    // Прямым текстом: ссылка НЕ зашифрована. Обратное утверждение на этом
    // экране появиться не может — шифровать её нечем.
    expect(banners(), contains('не зашифрована'));
    // И единственный баннер говорит про ссылку, а не про сверку адреса.
    expect(banners(), isNot(contains('сертификат')));
  });
