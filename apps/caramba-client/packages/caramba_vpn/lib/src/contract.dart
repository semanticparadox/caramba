...
///
/// Ошибки ядра приезжают текстом и переводятся в `core_error.dart`; эти
/// придумывает клиентская сторона, поэтому они машинные и стабильные. Первые
/// две печатает ещё и нативный мост (`CarambaFailureReason` в Kotlin) — один и
/// тот же литерал по обе стороны провода; третью не печатает никто, кроме
/// приложения: провод в ней не участвует, там решение принимается по молчанию
/// провода.
abstract final class VpnFailureReason {
  ...
  /// Команда «опустить туннель» ушла, а кадра о том, что ядро остановлено, за
  /// отведённое время не пришло. Поднимать на этом месте нельзя: подъём попал
  /// бы в ещё не закончившуюся разборку, а это и есть «Защищено» над мёртвым
  /// туннелем.
  static const String stopNotConfirmed = 'stop_not_confirmed';
}
