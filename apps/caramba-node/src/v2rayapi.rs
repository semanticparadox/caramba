// СГЕНЕРИРОВАНО из proto/stats.proto — правки руками будут потеряны.
//
// Файл закоммичен намеренно, а не собирается через build.rs: tonic-build
// требует protoc, а релизный конвейер собирает musl-бинарники в контейнере,
// куда protoc пришлось бы отдельно ставить. Контракт StatsService не меняется
// годами, поэтому дешевле хранить результат.
//
// Перегенерировать (нужен protoc):
