...
pub fn t(lang: Option<&str>, key: &str) -> &'static str {
    ...
    match (key, is_ru) {
        ...
        ("kb.guides", true) => "📖 Как подключить",
        ("kb.guides", false) => "📖 How to connect",
        ("kb.guide_index", true) => "📖 Пошаговая инструкция",
        ("kb.guide_index", false) => "📖 Step-by-step guide",
        ("kb.guide_ios", true) => "iPhone и iPad",
        ("kb.guide_ios", false) => "iPhone and iPad",
        ("kb.guide_android", true) => "Android",
        ("kb.guide_android", false) => "Android",
        ("kb.guide_windows", true) => "Windows",
        ("kb.guide_windows", false) => "Windows",
        ("kb.guide_macos", true) => "Mac",
        ("kb.guide_macos", false) => "Mac",
        ("kb.guide_linux", true) => "Linux",
        ("kb.guide_linux", false) => "Linux",
        ("kb.guide_tv", true) => "Android TV",
        ("kb.guide_tv", false) => "Android TV",
        ("kb.guide_router", true) => "Роутер и ручная настройка",
        ("kb.guide_router", false) => "Router and manual setup",
        ("msg.guides_prompt", true) => "Выберите устройство — инструкция займёт две минуты. Одна подписка работает на всех устройствах в пределах лимита тарифа.",
        ("msg.guides_prompt", false) => "Pick your device — the guide takes two minutes. One subscription works on all your devices within the plan limit.",
        ("msg.guides_missing", true) => "Инструкции ещё не опубликованы. Напишите в поддержку — поможем подключиться.",
        ("msg.guides_missing", false) => "Guides are not published yet. Contact support and we will help you connect.",
