...
use crate::models::store::{User, Plan, SubscriptionIpTracking, DetailedSubscription, GiftCode};
        ...
        match data.as_str() {
            "set_lang_en" | "set_lang_ru" => {
                ...
                if let Ok(Some(u)) = user_res {
                    ...
                    let terms_text = state.settings.get_or_default("terms_of_service", "Terms of Service...").await;
            ...
            extend if extend.starts_with("extend_sub_") => {
                    let text_plans: Vec<Plan> = state.store_service.get_active_plans().await.unwrap_or_default();
            ...
            cb if cb.starts_with("cb_") => {
                ...
                if let Ok(Some(u)) = user_res {
                    match state.pay_service.create_cryptobot_invoice(u.id, amount, PaymentType::BalanceTopup).await {
                        Ok(url) => {
                             let buttons = vec![vec![InlineKeyboardButton::url("🔗 Pay with CryptoBot", url.parse::<Url>().unwrap())]];
                             let _ = bot.answer_callback_query(callback_id).await;
                             if let Some(msg) = q.message {
            ...
            np if np.starts_with("np_") => {
                ...
                if let Ok(Some(u)) = user_res {
                    match state.pay_service.create_nowpayments_invoice(u.id, amount, PaymentType::BalanceTopup).await {
                        Ok(url) => {
                             let buttons = vec![vec![InlineKeyboardButton::url("🔗 Pay with NOWPayments", url.parse::<Url>().unwrap())]];
                             let _ = bot.answer_callback_query(callback_id).await;
                             if let Some(msg) = q.message {
                                 let _ = bot.send_message(msg.chat().id, format!("💳 Invoice for *${:.2}* created\\!", amount)).parse_mode(ParseMode::MarkdownV2).reply_markup(InlineKeyboardMarkup::new(buttons)).await;
                             }
            ...
            cp if cp.starts_with("cp_") => {
                ...
                if let Ok(Some(u)) = user_res {
                    match state.pay_service.create_crystalpay_invoice(u.id, amount, PaymentType::BalanceTopup).await {
                        Ok(url) => {
                             let buttons = vec![vec![InlineKeyboardButton::url("🔗 Pay with Card (CrystalPay)", url.parse::<Url>().unwrap())]];
                             let _ = bot.answer_callback_query(callback_id).await;
                             if let Some(msg) = q.message {
                                 let _ = bot.send_message(msg.chat().id, format!("💳 Invoice for *${:.2}* created\\!", amount)).parse_mode(ParseMode::MarkdownV2).reply_markup(InlineKeyboardMarkup::new(buttons)).await;
                             }
            ...
            str_pay if str_pay.starts_with("str_") => {
                ...
                if let Ok(Some(u)) = user_res {
                    match state.pay_service.create_stripe_session(u.id, amount, PaymentType::BalanceTopup).await {
                        Ok(url) => {
                             let buttons = vec![vec![InlineKeyboardButton::url("🔗 Pay with Stripe", url.parse::<Url>().unwrap())]];
                             let _ = bot.answer_callback_query(callback_id).await;
                             if let Some(msg) = q.message {
                                 let _ = bot.send_message(msg.chat().id, format!("💳 Invoice for *${:.2}* created\\!", amount)).parse_mode(ParseMode::MarkdownV2).reply_markup(InlineKeyboardMarkup::new(buttons)).await;
                             }
            ...
            get_links if get_links.starts_with("get_links_") => {
                    ...
                    let _ = bot.answer_callback_query(callback_id.clone()).text("Fetching links...").await;

                    let links_res: AnyhowResult<Vec<String>> = state.store_service.get_subscription_links(sub_id).await;
                    match links_res {
                        Ok(links) => {
                            if links.is_empty() {
                                let _ = bot.send_message(ChatId(tg_id), "❌ No connection links available.").await;
                            } else {
                                let mut response = "🔗 *Your Connection Links:*\n\n".to_string();
                                for link in links {
                                    response.push_str(&format!("`{}`\n\n", escape_md(&link)));
                                }
                                let _ = bot.send_message(ChatId(tg_id), response).parse_mode(ParseMode::MarkdownV2).await;
                            }
                        }
                        Err(e) => {
                            error!("Links error: {}", e);
                            let _ = bot.send_message(ChatId(tg_id), "❌ Failed to fetch links.").await;
                        }
                    }
            }
                ...
                if let Ok(Some(u)) = user_res {
                    match state.store_service.generate_subscription_file(u.id).await {
                        ...
                        Err(e) => {
                             ...
                             let _ = bot.send_message(ChatId(tg_id), "❌ Failed to generate profile.").await;
                        }
                    }
                }
            }
            ...
            "my_gifts" => {
                ...
                if let Ok(Some(u)) = user_res {
                    let _ = bot.answer_callback_query(callback_id).await;
                    let codes_res: AnyhowResult<Vec<GiftCode>> = state.store_service.get_user_gift_codes(u.id).await;
                    match codes_res {
                        Ok(codes) => {
                            if codes.is_empty() {
                                    if let Some(msg) = q.message {
            ...
            devices if devices.starts_with("devices_") => {
                 ...
                 let _ = bot.answer_callback_query(callback_id).await;
                  ...
                  let limit_res: AnyhowResult<i64> = state.store_service.get_subscription_device_limit(sub_id).await;
                  let limit = limit_res.unwrap_or(3);
                  let ips_res: AnyhowResult<Vec<SubscriptionIpTracking>> = state.store_service.get_subscription_active_ips(sub_id).await;
                  ...
                  let mut response = format!("📱 *CONNECTED DEVICES*\\n\\n");
                    ...
                    if ips.is_empty() {
                        ...
                        for (idx, ip_record) in ips.iter().take(10).enumerate() {
                            let duration = chrono::Utc::now().signed_duration_since(ip_record.last_seen_at);
                            let minutes_ago = duration.num_minutes();
                            response.push_str(&format!("{}\\. `{}` _{} min ago_\\n", idx + 1, escape_md(&ip_record.client_ip), minutes_ago));
                        }
                    }
            ...
            buy_plan_idx if buy_plan_idx.starts_with("buy_plan_idx_") => {
                 ...
                 let plans: Vec<Plan> = state.store_service.get_active_plans().await.unwrap_or_default();
                  ...
                  if !plans.is_empty() {
                      ...
                      let _ = bot.answer_callback_query(callback_id).text("❌ No active plans.").await;
                  }
            }
             ...
             myservices_page if myservices_page.starts_with("myservices_page_") => {
                ...
                if let Ok(Some(user)) = user_res {
                    let subs_res: AnyhowResult<Vec<DetailedSubscription>> = state.store_service.get_user_subscriptions(user.id).await;
                    if let Ok(subs) = subs_res {
                        ...
                        if !sorted_subs.is_empty() {
                            ...
                            if sub.sub.status == "active" {
                                ...
                                buttons.push(vec![InlineKeyboardButton::callback("📱 Devices", format!("devices_{}", sub.sub.id))]);
             ...
             buy_dur if buy_dur.starts_with("buy_dur_") => {
                let id = buy_dur.strip_prefix("buy_dur_").unwrap().parse::<i64>().unwrap_or(0);
                let user_res: AnyhowResult<Option<User>> = state.store_service.get_user_by_tg_id(tg_id).await;
                if let Ok(Some(u)) = user_res {
                    match state.store_service.purchase_plan(u.id, id).await {
                        Ok(_) => {
                             let _ = bot.answer_callback_query(callback_id).text("✅ Success!").await;
                             if let Some(msg) = q.message {
                                 let _ = bot.send_message(msg.chat().id, "✅ *Purchase Successful!*").parse_mode(ParseMode::MarkdownV2).await;
                             }
                        }
                        Err(e) => { let _ = bot.answer_callback_query(callback_id).text(format!("Error: {}", e)).show_alert(true).await; }
                    }
            ...
            ext_dur if ext_dur.starts_with("ext_dur_") => {
                 let id = ext_dur.strip_prefix("ext_dur_").unwrap().parse::<i64>().unwrap_or(0);
                 let user_res: AnyhowResult<Option<User>> = state.store_service.get_user_by_tg_id(tg_id).await;
                 if let Ok(Some(u)) = user_res {
                     match state.store_service.extend_subscription(u.id, id).await {
                         Ok(_) => {
                              let _ = bot.answer_callback_query(callback_id).text("✅ Extended!").await;
                              if let Some(msg) = q.message {
                                  let _ = bot.send_message(msg.chat().id, "✅ *Subscription Extended!*").parse_mode(ParseMode::MarkdownV2).await;
                              }
                         }
                         Err(e) => { let _ = bot.answer_callback_query(callback_id).text(format!("Error: {}", e)).show_alert(true).await; }
                     }
