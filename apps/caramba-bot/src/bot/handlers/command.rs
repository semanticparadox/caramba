...
pub async fn message_handler(
    ...
    if let Some(payment) = msg.successful_payment() {
         ...
         match state.pay_service.process_any_payment(amount_usd, "stars", Some(payment.provider_payment_charge_id.clone()), &payment.invoice_payload).await {
             Ok(_) => { 
                 ...
                 let _ = state.logging_service.log_user(
                     Some(tg_id),
                     "payment_stars",
    ...
    if let Some(text) = msg.text() {
        ...
        let user_res: Option<User> = if text.starts_with("/start") {
            ...
            // Upsert returns User
            let user_res_inner: AnyhowResult<Option<User>> = state.store_service.upsert_user(
                tg_id, 
            ...
            let res: AnyhowResult<Option<User>> = state.store_service.get_user_by_tg_id(tg_id).await;
            res.ok().flatten()
        };
        ...
        if let Some(user) = &user_res {
            ...
            if user.terms_accepted_at.is_none() {
                ...
                let terms_text: String = state.settings.get_or_default("terms_of_service", "Terms of Service...").await;
            ...
            if let Some(reply_text) = reply.text() {
                ...
                // Transfer
                    if reply_text.contains("Transfer Subscription") && reply_text.contains("Subscription #") {
                        ...
                        if let Ok(sub_id) = id_str.parse::<i64>() {
                            let user_db_res: AnyhowResult<Option<User>> = state.store_service.get_user_by_tg_id(tg_id).await;
        ...
        match text {
            ...
            "📦 Digital Store" => {
                    let categories_res: AnyhowResult<Vec<StoreCategory>> = state.store_service.get_categories().await;
            ...
            "🛒 My Cart" | "/cart" => {
                  if let Some(user) = &user_res {
                     let cart_items_res: AnyhowResult<Vec<CartItem>> = state.store_service.get_user_cart(user.id).await;
            ...
            "/devices" | "📱 My Devices" => {
                  if let Some(u) = &user_res {
                       if let Ok(subs) = state.store_service.get_user_subscriptions(u.id).await {
                           let active_subs: Vec<DetailedSubscription> = subs.into_iter().filter(|s| s.sub.status == "active").collect();
                           ...
                           if active_subs.is_empty() {
                               ...
                               let ips_res: AnyhowResult<Vec<SubscriptionIpTracking>> = state.store_service.get_subscription_active_ips(sub.sub.id).await;
                               let ips = ips_res.unwrap_or_default();
                               let limit_res: AnyhowResult<i64> = state.store_service.get_subscription_device_limit(sub.sub.id).await;
                               ...
                               if ips.is_empty() {
                                   ...
                                   for ip in &ips {
                                        ...
                                        let duration = chrono::Utc::now().signed_duration_since(ip.last_seen_at);
                               ...
                               let _ = bot.send_message(msg.chat.id, "📱 *Select a subscription to view devices:*")
                                   .parse_mode(ParseMode::MarkdownV2)
                                   .reply_markup(InlineKeyboardMarkup::new(buttons))
                                   .await;
                           }
