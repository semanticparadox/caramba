...
class VisualEditor {
    ...
    init() {
        ...
        this.container.querySelector('.vis-sniff')?.addEventListener('change', () => this.updateVisualState());
        this.container.querySelector('.vis-sniff-override')?.addEventListener('change', () => this.generateJsonFromVisual());
        this.container.querySelector('.vis-multiplex')?.addEventListener('change', () => this.generateJsonFromVisual());

        // Routing Mode
        this.container.querySelectorAll('input[name="vis_routing"]').forEach(el => {
            el.addEventListener('change', () => this.generateJsonFromVisual());
        });
    ...
    updateVisualState() {
        ...
        // Sniffing Override Logic (Only show if sniffing is enabled)
        const sniffEnabled = this.container.querySelector('.vis-sniff')?.checked;
        toggle('.vis-sniff-override-container', sniffEnabled);
    ...
    generateJsonFromVisual() {
        ...
        let stream = {
            network: network,
            security: security,
            sniff: this.container.querySelector('.vis-sniff')?.checked || false,
            sniff_override_destination: this.container.querySelector('.vis-sniff-override')?.checked || false,
        };

        // Routing Mode
        const routingMode = this.container.querySelector('input[name="vis_routing"]:checked')?.value || 'smart';
        stream.routing_mode = routingMode;

        // Multiplex
        if (this.container.querySelector('.vis-multiplex')?.checked) {
            stream.multiplex = {
                enabled: true,
                protocol: "smux",
                max_connections: 4,
            ...
            // Optional: Explicitly disable or omit? Omit usually implies default (disabled) or inherited.
            // Let's omit if unchecked to keep JSON clean.
        }
    ...
    parseJsonToVisual() {
        try {
            ...
            // Advanced Options
            if (stream.sniff !== undefined) {
                const el = this.container.querySelector('.vis-sniff');
                if (el) el.checked = stream.sniff;
            }
            if (stream.sniff_override_destination !== undefined) {
                const el = this.container.querySelector('.vis-sniff-override');
                if (el) el.checked = stream.sniff_override_destination;
            }
            if (stream.multiplex && stream.multiplex.enabled) {
                const el = this.container.querySelector('.vis-multiplex');
            ...
            if (stream.routing_mode) {
                const el = this.container.querySelector(`input[name="vis_routing"][value="${stream.routing_mode}"]`);
                if (el) el.checked = true;
            }
