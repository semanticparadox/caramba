...
// инициализации. Любой из параметров может быть пустым; пустые получают значения
// по умолчанию. Параметры — C-строки UTF-8.
//
// Пустой panelURL — законное состояние, а не недосмотр: это режим импортированной
// подписки, где панели нет вовсе. Ровно на вере в обратное держалась заглушка
// `https://panel.invalid`, из-за которой пресет блокировки рекламы считал, что у
// оператора есть зеркало правил, подавлял откат на встроенный тег и выпускал
// провайдера по адресу, который никогда не резолвится. Не возвращайте сюда
// «обязателен»: непустая база означает «зеркало есть», и выдумывать её нельзя.
//
//export CarambaNew
...
// CarambaRouteReport отдаёт отчёт о маршрутизации последнего подъёма: пресет,
// судьбу каждого его внешнего списка с причиной отказа, теги GEOSITE и
// доступность базы GeoSite.dat, а также судьбу выбранной страны входа.
//
// Читающий вызов без сети.
//
//export CarambaRouteReport
func CarambaRouteReport(h C.long) *C.char {
	cl := lookup(h)
	if cl == nil {
		return errJSON("неизвестный хэндл")
	}
	return okOrErr(cl.RouteReport())
}
