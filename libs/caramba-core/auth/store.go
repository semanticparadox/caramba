...
type Tokens struct {
	...
	// срабатывает только по ответу 401.
	AccessExpiry time.Time `json:"access_expiry,omitempty"`
}

// Valid сообщает, есть ли пригодный для запросов токен.
//
// Достаточно непустого access-токена: им подписываются авторизованные запросы.
// refresh-токен нужен лишь для авто-обновления по истечении/401 — его отсутствие
// не делает текущую сессию неавторизованной, а лишь лишает её авто-продления
// (см. SetTokens: инъекция access-only — допустимый, деградирующий режим). Все
// сетевые входы (Login*/Register/Refresh) возвращают обе части, поэтому для них
