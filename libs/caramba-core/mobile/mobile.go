...
// RouteReport отдаёт то, что ПОСЛЕДНИЙ подъём реально применил к
// маршрутизации: пресет, судьбу каждого его внешнего списка (доехал файлом,
// доехал адресом зеркала или выброшен и почему), теги GEOSITE, от которых
// пресет зависит, доступность базы GeoSite.dat и судьбу выбранной страны входа.
//
// Экран настроек до этого символа не имел НИ ОДНОГО способа отличить
// работающий блок рекламы от включённого и мёртвого: правила GEOSITE без базы
// не значат ничего, а сборка конфига молча выбрасывает недоступный список
// вместе со ступившими на него правилами. «Неизвестно» здесь законный ответ и
// приходит с причиной.
func (c *Client) RouteReport() (string, error) { return c.core.RouteReportJSON() }
